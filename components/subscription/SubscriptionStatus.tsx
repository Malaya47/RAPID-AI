...
export function SubscriptionStatus() {
  ...
  const handleCancelSubscription = async () => {
    setLoading(true);
    try {
      // Fetch the Stripe customer ID from the subscription or user object
      const customerId = profile?.stripe_customer_id;
      if (!customerId) {
        alert("Stripe customer ID not found.");
        setLoading(false);
        return;
      }
      ...
      alert("Error redirecting to Stripe portal.");
    }
    setLoading(false);
  };
    ...
    return (
      <Card className="bg-neutral-950 border-none shadow-md shadow-neutral-300 text-white">
        <CardHeader>
          ...
          <CardDescription>
            You do not have an active subscription. Subscribe to a plan to start
            using credits.
          </CardDescription>
        </CardHeader>
        <CardContent>
